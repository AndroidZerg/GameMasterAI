...
async def subscribe_venue(venue_id: str, req: SubscribeRequest,
    ...
    current_limit = venue.get("game_seat_limit") or 10
    ...
    if not stripe_customer_id:
        ...
        update_venue_fields(venue_id, {"stripe_customer_id": stripe_customer_id})

    # --- New subscription (Stripe Checkout) ---
    if not venue.get("stripe_subscription_id"):
    ...
    update_venue_fields(venue_id, {
        ...
        "game_seat_limit": new_seat_limit,
    })
...
async def subscription_status(venue_id: str, user: dict = Depends(get_current_venue_admin)):
    ...
    trial_ends_at = None
    if venue.get("stripe_subscription_id") and STRIPE_SECRET_KEY:
        try:
            sub = stripe.Subscription.retrieve(venue["stripe_subscription_id"])
            if sub.trial_end:
    ...
    return {
        ...
        "subscription_status": venue.get("subscription_status") or "trialing",
        "current_period_end": venue.get("current_period_end"),
