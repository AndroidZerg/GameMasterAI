"""Download real BGG cover art for games D-M only."""
import requests
import json
import time
import os
...
for fname in sorted(os.listdir(games_dir)):
    ...
    # Filter: only D-M (first char)
    first_char = game_id[0].lower()
    if first_char < 'd' or first_char > 'm':
        continue
...
print(f"Found {len(games)} games (D-M) to process")
for g in games:
    print(f"  {g['game_id']}")

def search_bgg(title):
    """Search BGG by title and return the best matching game ID"""
    try:
...
for i, game in enumerate(games):
    ...
    # Download (overwrite placeholder)
...
# Save verified IDs for D-M batch
with open("content/bgg-ids-dm.json", "w") as f:
    json.dump(verified_ids, f, indent=2)

# Summary
print(f"\n{'='*60}")
print(f"RESULTS (D-M batch):")
