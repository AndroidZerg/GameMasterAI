"""Download real BGG cover art for games N-Z only."""
import requests
import json
import time
import os
...
print(f"Found {len(games)} games (N-Z) to process", flush=True)
for g in games:
    print(f"  {g['game_id']}")
...
# Save verified IDs for N-Z batch
with open("content/bgg-ids-nz.json", "w") as f:
    json.dump(verified_ids, f, indent=2)

# Summary
print(f"\n{'='*60}")
print(f"RESULTS (N-Z batch):")
