...
- Backend serves via `/api/images/{filename}` from `images.py`
- Path resolution: `backend/app/api/routes/images.py` → parents[4] → `content/images/` ✓
- Frontend requests `${API_BASE}/api/images/${game.game_id}.jpg` with fallback to colored bg
- All 50 images verified present and path-accessible
